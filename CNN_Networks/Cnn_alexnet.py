...
def conv_net(x, weights, biases):
    ...
    # Convolution Layer
    conv1 = conv2d(x, weights['wc1'], biases['bc1'],strides=4)#254 with filter of 12x12
    # Max Pooling (down-sampling)
    conv1 = maxpool2d(conv1,k=2,strides=3)#85
    
     # Convolution Layer
    conv2 = conv2d(conv1, weights['wc2'], biases['bc2'],strides=2,padding=2)#85 with filter of 5x5
    # Max Pooling (down-sampling)
    conv2 = maxpool2d(conv2,k=3,strides=2)#42 

    conv3 = conv2d(conv2, weights['wc3'], biases['bc3'],strides=1,padding=1)#42 with filter of 3x3
    conv4 = conv2d(conv3, weights['wc4'], biases['bc4'],strides=1,padding=1)#42 with filter of 3x3
    conv5 = conv2d(conv4, weights['wc5'], biases['bc5'],strides=1,padding=1)#42 with filter of 3x3

    conv5 = maxpool2d(conv5,k=3,strides=3)#14 (14*14*60(kernel size) = )
...
weights = {
    ...
    'wc1': tf.Variable(tf.random_normal([12, 12, 1, 24])),
    'wc2': tf.Variable(tf.random_normal([5, 5, 24, 60])),
    'wc3': tf.Variable(tf.random_normal([3, 3, 60, 96])),
    'wc4': tf.Variable(tf.random_normal([3, 3, 96, 96])),
    'wc5': tf.Variable(tf.random_normal([3, 3, 96, 60])),
    # fully connected, 7*7*32 inputs, 1024 outputs
    'wd1': tf.Variable(tf.random_normal([14*14*60, 4096])),
...
biases = {
    ...
    'bc2': tf.Variable(tf.random_normal([66])),
    'bc3': tf.Variable(tf.random_normal([96])),
    'bc4': tf.Variable(tf.random_normal([96])),
    'bc5': tf.Variable(tf.random_normal([60])),
