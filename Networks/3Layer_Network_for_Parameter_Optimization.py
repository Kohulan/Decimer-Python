...
hidden_neurons_list_I = [2,4,8,16,32,64,128,256,512,1024,2048,4096]
...
for hidden_neurons_I in range(len(hidden_neurons_list_I)):
	for batch_sizer in range(len(batch_sizer_list)):
		for learning_rate_ in range(len(learning_rate_list)):
			f = open("/Results/Batch Size_{}_learning_rate_{}_hidden_neurons_{}.txt".format(batch_sizer_list[batch_sizer],learning_rate_list[learning_rate_],hidden_neurons_list_I[hidden_neurons_I]), 'w',0)
			sys.stdout = f
			...
			def label_data(is_test=False):
				...
				if is_test:
					data_path = "test"
				myFile = open('/Data/New_'+data_path+'_labels.csv',"r")
				labels = []
				for row in myFile:
			...
			#Image array data
			Train_Images = pickle.load( open("/Data/Potential_train_compressed.txt","rb"))
			Test_Images = pickle.load( open("/Data/Potential_test_compressed.txt","rb"))
			...
			with tf.Session(config=config) as sess:
				...
				print("\n---------------------------------------------------------------------------------------------------------------------------------------------------------------------\n")
				print("Batch Size_{}_learning_rate_{}_hidden_neurons_{}.txt".format(batch_sizer_list[batch_sizer],learning_rate_list[learning_rate_],hidden_neurons_list_I[hidden_neurons_I]))
			...
			plt.plot(test_loss_history, '-o', label='Loss value')
			...
			plt.savefig("/Results/Batch Size_{}_learning_rate_{}_hidden_neurons_{}.png".format(batch_sizer_list[batch_sizer],learning_rate_list[learning_rate_],hidden_neurons_list_I[hidden_neurons_I]))
			plt.close()