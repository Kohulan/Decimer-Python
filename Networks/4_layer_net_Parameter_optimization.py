	...
	for hidden_neurons_II in range(len(hidden_neurons_list_II)):
		for batch_sizer in range(len(batch_sizer_list)):
			for learning_rate_ in range(len(learning_rate_list)):
				f = open("Path+to+folder/Batch Size_{}_learning_rate_{}_hidden_neurons_{}_x_{}.txt".format(batch_sizer_list[batch_sizer],learning_rate_list[learning_rate_],hidden_neurons_list_I[hidden_neurons_I],hidden_neurons_list_II[hidden_neurons_II]), 'w',0)
				sys.stdout = f
				...
				def label_data(is_test=False):
					...
					if is_test:
						data_path = "test"
					myFile = open('My_'+data_path+'_labels.csv',"r")
					labels = []
					for row in myFile:
				...
				with tf.Session(config=config) as sess:
					...
					Y_test = y_test_enc
					test_acc = accuracy.eval({X: X_test, Y: Y_test})
					print (datetime.now().strftime('%Y/%m/%d %H:%M:%S'),"Final Test Accuracy:", test_acc)
					_, predict,prediction_difference = sess.run([loss_op,pred,pred_difference], feed_dict={X: X_test, Y: Y_test})
					prediction_window = np.absolute(prediction_difference)
					print(len(prediction_window))
					for j in range(10):
						count_error = 0
				...
				plt.savefig("Path+to+folder/Batch Size_{}_learning_rate_{}_hidden_neurons_{}_x_{}.png".format(batch_sizer_list[batch_sizer],learning_rate_list[learning_rate_],hidden_neurons_list_I[hidden_neurons_I],hidden_neurons_list_II[hidden_neurons_II]))
				plt.close()